...
export async function GET(request) {
  try {
    ...
    if (!filmUrl) {
      return new Response(
        JSON.stringify({ message: 'Film URL is required' }),
        { status: 400 }
      );
    }
...
export async function POST(request) {
    try {
      const { filmUrl, genre } = await request.json(); // Use filmUrl instead of slug
...
export async function DELETE(request) {
    try {
      ...
      if (!filmUrl || !genre) {
        return new Response(
          JSON.stringify({ message: 'Film URL and genre are required' }),
          { status: 400 }
        );
      }
