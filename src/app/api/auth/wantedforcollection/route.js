...
// POST: add to wantedforcollection or set iswatched true
export async function POST(req, { params }) {
  ...
  try {
    ...
    await sql`
      INSERT INTO wantedforcollection (username, url, iswatched)
      VALUES (${username}, ${url}, true)
      ON CONFLICT (username, url)
      DO UPDATE SET iswatched = true;
    `;
    return new Response(JSON.stringify({ message: 'Watch added' }), { status: 200 });
  } catch (err) {
    return new Response(JSON.stringify({ message: 'Server error', error: err.message }), { status: 500 });
  }
}

// DELETE: mark iswatched false (or remove row)
