  ...
  useEffect(() => {
    ...
    const fetchWantedMovies = async () => {
      try {
        ...
        // Filter for only wanted movies
        const wanted = userMovies.filter((m) => m.is_wanted === true);
        setMovies(wanted);
      } catch (err) {
        console.error("Error fetching wanted movies:", err);
    ...
    <Container className="py-4">
      <div className={styles.hero}>
        <h1 className={styles.heroTitle}>
          Wanted for {username}&apos;s Collection
        </h1>
      </div>
