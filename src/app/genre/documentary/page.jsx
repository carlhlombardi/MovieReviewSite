...
const DocumentaryPostPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('https://movie-review-site-seven.vercel.app/api/data/documentarymovies');
  ...
  return (
    <Container>
      ...
      <Col>
           <Image
           src={"/images/hero/Documentary.jpg"} // Use the image URL directly from the database
        ...
        <Row>
          {itemsToShow.map(item => (
            <Col key={item.row_id} xs={12} sm={6} md={4} lg={3}>
              <Link href={`/genre/documentary/${encodeURIComponent(item.url)}`}>
...
export default DocumentaryPostPage;
